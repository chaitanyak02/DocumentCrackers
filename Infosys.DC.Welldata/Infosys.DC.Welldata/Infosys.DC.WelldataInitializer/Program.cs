...
namespace Infosys.DC.WelldataInitializer
{
    class Program
    {
        ...
        private static async Task<bool> RunAsync()
        {
            ...
            // Implement logic to create additional indexer for cosmos db and merge the documents 
            if (!result)
                return result;
            ...
            await CreateAndRunCosmosDbIndexer("welldata");
            await CreateAndRunCosmosDbIndexer("wellinfo");
            await CreateAndRunCosmosDbIndexer("sundryinfo");
            if (!result)
                return result;
        ...
        private static async Task<bool> DeployWebsite()
        {
            try
            {
                ...
                string envText = File.ReadAllText("../../../../UILayer/.env");
                ...
                File.WriteAllText("../../../../UILayer/.env", envText);
                ...
                ZipFile.CreateFromDirectory("../../../../UILayer/dist", "website.zip");
        ...
        private static async Task CreateAndRunCosmosDbIndexer()
        {
            // Append the database name to the connection string
            string cosmosConnectString = ConfigurationManager.AppSettings["CosmosDBConnectionString"]
                + ";Database="
                + ConfigurationManager.AppSettings["CosmosDBDatabaseName"];

            DataSource cosmosDbDataSource = DataSource.CosmosDb(
                name: ConfigurationManager.AppSettings["CosmosDBDatabaseName"],
                cosmosDbConnectionString: cosmosConnectString,
                collectionName: "welldata",
                useChangeDetection: true);

            // The Cosmos DB data source does not need to be deleted if it already exists,
            // but the connection string might need to be updated if it has changed.
            await _searchClient.DataSources.CreateOrUpdateAsync(cosmosDbDataSource);
            ...
            bool exists = await _searchClient.Indexers.ExistsAsync(cosmosDbIndexer.Name);
            if (exists)
            {
                await _searchClient.Indexers.ResetAsync(cosmosDbIndexer.Name);
            }
            await _searchClient.Indexers.CreateOrUpdateAsync(cosmosDbIndexer);

            Console.WriteLine("Running Cosmos DB indexer...\n");

            try
            {
                await _searchClient.Indexers.RunAsync(cosmosDbIndexer.Name);
            }
        ...
        private static async Task CreateAndRunCosmosDbIndexer(string collection)
        {
            // Append the database name to the connection string
            string cosmosConnectString = ConfigurationManager.AppSettings["CosmosDBConnectionString"]
                + ";Database="
            ...
            Indexer cosmosDbIndexer = new Indexer(
                name: collection+"-cosmosdb-indexer",
                dataSourceName: cosmosDbDataSource.Name,
                targetIndexName: IndexName,
                fieldMappings: map,
                schedule: new IndexingSchedule(TimeSpan.FromDays(1)));
        ...
        private static async Task CreateAndRunCosmosDbIndexer(string collection, string idfield)
        {
            // Append the database name to the connection string
            string cosmosConnectString = ConfigurationManager.AppSettings["CosmosDBConnectionString"]
                + ";Database="
